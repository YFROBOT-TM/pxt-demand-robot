/** 
 * @file pxt-demand-robot/main.ts
 * @brief DEMAND ROBOT makecode library.
 * @n This is a MakeCode graphical programming education robot.
 * 
 * @copyright    DEMAND,2021
...
// ultrasonic pin
let demandRobotUltrasonicTrig = DigitalPin.P5
let demandRobotUltrasonicEcho = DigitalPin.P11
let demandRobot_distanceBuf = 0
// motor pin 
let demandRobotMotorLD = DigitalPin.P13
let demandRobotMotorLA = AnalogPin.P14
let demandRobotMotorRD = DigitalPin.P15
let demandRobotMotorRA = AnalogPin.P16
// patrol pin
let demandRobotPatrolLeft = DigitalPin.P1
let demandRobotPatrolMiddle = DigitalPin.P2
let demandRobotPatrolRight = DigitalPin.P8
// rgbled pin
let demandRobotEyesPin = DigitalPin.P11
...
namespace demandRobot {
     ...
     */
    //% weight=120
    //% blockId=demandRobot_writeLED block="LEDlight |%ledn turn |%ledswitch"
     ...
     */
    //% weight=90
    //% blockId=demandRobot_motor_MotorRun block="motor|%index|move|%direction|at speed|%speed"
    ...
    export function motorRun(index: Motors, direction: Dir, speed: number): void {
        ...
        if (index == demandRobot.Motors.ML) {
            pins.digitalWritePin(demandRobotMotorLD, direction);
            pins.analogWritePin(demandRobotMotorLA, speed);
        } else if (index == Motors.MR) {
            pins.digitalWritePin(demandRobotMotorRD, direction);
            pins.analogWritePin(demandRobotMotorRA, speed);
     ...
     * @param motor motor ML/MR/MAll. eg: demandRobot.Motors.MAll
     */
    //% weight=89
    //% blockId=demandRobot_motor_motorStop block="motor |%motor stop"
     ...
     */
    //% blockId=demandRobot_ultrasonic_sensor block="read ultrasonic sensor |%unit "
    //% weight=80
    export function Ultrasonic(unit: PingUnit, maxCmDistance = 500): number {
        // send pulse
        pins.setPull(demandRobotUltrasonicTrig, PinPullMode.PullNone);
        pins.digitalWritePin(demandRobotUltrasonicTrig, 0);
        control.waitMicros(2);
        pins.digitalWritePin(demandRobotUltrasonicTrig, 1);
        control.waitMicros(10);
        pins.digitalWritePin(demandRobotUltrasonicTrig, 0);

        // read pulse
        // d = pins.pulseIn(demandRobotUltrasonicEcho, PulseValue.High, maxCmDistance * 58);  // 8 / 340 = 
        let d = pins.pulseIn(demandRobotUltrasonicEcho, PulseValue.High, 25000);
        let ret = d;
        // filter timeout spikes
        if (ret == 0 && demandRobot_distanceBuf != 0) {
            ret = demandRobot_distanceBuf;
        }
        demandRobot_distanceBuf = d;

        return Math.floor(ret * 9 / 6 / 58);
        // switch (unit) {
        //     case demandRobotPingUnit.Centimeters: return Math.idiv(d, 58);
        //     default: return d;
        // }
    }
    ...
    /**
      ...
      * @param enable line tracking sensor enable signal(0 or 1), eg: demandRobot.PatrolEnable.PatrolOn
      */
    //% weight=71
    //% blockId=demandRobot_Patrol_enable block="%enable line tracking sensor"
    ...
    /**
      ...
      */
    //% weight=70
    //% blockId=demandRobot_read_Patrol block="read %patrol line tracking sensor"
    //% patrol.fieldEditor="gridpicker" patrol.fieldOptions.columns=2 
    export function readPatrol(patrol: Patrol): number {
        if (patrol == Patrol.PatrolLeft) {
            return pins.digitalReadPin(demandRobotPatrolLeft)
        } else if (patrol == Patrol.PatrolMiddle) {
            return pins.digitalReadPin(demandRobotPatrolMiddle)
        } else if (patrol == Patrol.PatrolRight) {
            return pins.digitalReadPin(demandRobotPatrolRight)
        } else {
            return -1
        }
    }
    ...
    export class Strip {
         ...
         */
        //% blockId="demandRobot_set_eyes_color" block="%eyes|show color at %eyes_n|to %rgb=neopixel_colors"
        //% eyes.defl=eyes
        //% weight=60
         ...
         */
        //% blockId="demandRobot_eyes_set_brightness" block="%eyes|set brightness %brightness" 
        //% eyes.defl=eyes
        //% weight=58
         ...
         */
        //% blockId="demandRobot_eyes_clear" block="%eyes|clear"
        //% eyes.defl=eyes
        //% weight=55
        clear(): void {
     ...
     *  @param mode rgb mode, eg: demandRobot.EyesMode.RGB
     */
    //% blockId="demandRobot_eyes_create" block="RGBEyes init %numleds|leds as %mode"
    //% weight=62  
    ...
    export function create(numleds: number, mode: EyesMode): Strip {
        ...
        eyes.setPin(demandRobotEyesPin)
        return eyes;
    }
    ...
    */
     ...
     * @param protocol IR protocol. eg: demandRobot.IrProtocol.NEC
     */
     ...
     */
    //% subcategory="IR_Receiver"
    //% blockId=demandRobot_infrared_on_ir_button
     ...
     */
    //% subcategory="IR_Receiver"
    //% blockId=demandRobot_infrared_ir_button_pressed
    //% block="IR button"
    //% weight=10
     ...
     */
    //% subcategory="IR_Receiver"
    //% blockId=demandRobot_infrared_was_any_button_pressed
     ...
     */
    //% subcategory="IR_Receiver"
    //% blockId=demandRobot_infrared_button_code
    //% button.fieldEditor="gridpicker"
    //% button.fieldOptions.columns=3
    //% button.fieldOptions.tooltips="false"
